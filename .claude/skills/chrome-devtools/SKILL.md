...
#### WSL2 Users: Use Windows Chrome (Recommended)

If you're on WSL2, you can connect to Chrome running on Windows instead of installing Chrome in WSL2. This offers better performance and GPU acceleration.

**See [WSL2-SETUP.md](./WSL2-SETUP.md) for complete WSL2 + Windows Chrome setup guide.**

**WSL2 Quick Start:** Source the helper functions for easy Chrome control:
```bash
source ~/projects/terminal-tabs/.claude/skills/chrome-devtools/chrome-helpers.sh
chrome-start  # Start Chrome with debugging
chrome-help   # Show all available commands
```

Add to `~/.bashrc` for persistent access:
```bash
echo "source ~/projects/terminal-tabs/.claude/skills/chrome-devtools/chrome-helpers.sh" >> ~/.bashrc
```

#### Step 1: Install System Dependencies (Linux only)

On native Linux, Chrome requires system libraries. Install them first:
...
**macOS/Windows/WSL2**: Skip this step
