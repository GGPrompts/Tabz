...
2. **"MCP servers"**
   - **Description**: "List available MCP servers from config and loaded tools"
...
- MCP: [server-name] ([description from detection])

[If Skills selected:]
- Skill: [skill-name] ([description from .claude/skills/])

[If Subagents selected:]
- Subagent: [agent-name] ([suggested use case based on task])
